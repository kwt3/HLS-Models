...
`define AUTOTB_DUT      decade_counter
`define AUTOTB_DUT_INST AESL_inst_decade_counter
`define AUTOTB_TOP      apatb_decade_counter_top
`define AUTOTB_LAT_RESULT_FILE "decade_counter.result.lat.rb"
`define AUTOTB_PER_RESULT_TRANS_FILE "decade_counter.performance.result.transaction.xml"
...
`define AUTOTB_TVIN_reset  "../tv/cdatafile/c.decade_counter.autotvin_reset.dat"
`define AUTOTB_TVIN_slowena  "../tv/cdatafile/c.decade_counter.autotvin_slowena.dat"
`define AUTOTB_TVIN_reset_out_wrapc  "../tv/rtldatafile/rtl.decade_counter.autotvin_reset.dat"
`define AUTOTB_TVIN_slowena_out_wrapc  "../tv/rtldatafile/rtl.decade_counter.autotvin_slowena.dat"
`define AUTOTB_TVOUT_out_r  "../tv/cdatafile/c.decade_counter.autotvout_out_r.dat"
...
// The signal of port slowena
reg [0: 0] AESL_REG_slowena = 0;
assign slowena = AESL_REG_slowena;
initial begin : read_file_process_slowena
    integer fp;
    integer err;
    integer ret;
    integer proc_rand;
    ...
    transaction_idx = 0;
    wait(AESL_reset === 0);
    fp = $fopen(`AUTOTB_TVIN_slowena,"r");
    if(fp == 0) begin       // Failed to open file
        $display("Failed to open file \"%s\"!", `AUTOTB_TVIN_slowena);
        $display("ERROR: Simulation using HLS TB failed.");
        $finish;
    end
    read_token(fp, token);
    ...
    while (token != "[[[/runtime]]]") begin
        ...
        if(token != "[[/transaction]]") begin
            ret = $sscanf(token, "0x%x", AESL_REG_slowena);
              if (ret != 1) begin
                  $display("Failed to parse token!");
                $display("ERROR: Simulation using HLS TB failed.");
                  $finish;
...
reg end_slowena;
reg [31:0] size_slowena;
reg [31:0] size_slowena_backup;
reg end_out_r;
reg [31:0] size_out_r;
